...
class Node(GameState):
    ...
    def unvisited_children(self):
        ...
        for i in range(len(self.matrix)):
            for j in range(len(self.matrix[0])):
                ...
                #     self.matrix[i][j] = self.make_updated_node(self.friend_transitions, self.enemy_transitions, self)
    ...
    #     """
    #     Initialize matrix and regret array with correct sizes
    #     """
    #     self.friend_transitions = self.next_transitions_for_side(is_friend=True)
    #     self.enemy_transitions = self.next_transitions_for_side(is_friend=False)
    #     self.matrix = [
    #         [None for _ in range(len(self.enemy_transitions))]
    #         for _ in range(len(self.friend_transitions))]

    #     self.regret = [0 for _ in range(len(self.friend_transitions))]
